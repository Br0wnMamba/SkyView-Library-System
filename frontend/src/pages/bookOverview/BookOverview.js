...
const BookOverview = () => {
	...
	return (
		<div>
			{showLogin ? (
				<div>
					<Login backgroundLocation={{ pathname: `/book/${id}` }} />
				</div>) : (
				<div className="book-overview-container">
					...
					<div className="book-overview-checkout">
						...
						<div className="book-overview-checkout-buttons">
							<button className={
								bookTypeCheckout === "physical" ? "book-overview-checkout-book-type selected" : "book-overview-checkout-book-type"
							} onClick={() => setBookTypeCheckout("physical")}>
								Paperback
							</button>
							<button className={
								bookTypeCheckout === "ebook" ? "book-overview-checkout-book-type selected" : "book-overview-checkout-book-type"
							} onClick={() => setBookTypeCheckout("ebook")}>
								E-Book
							</button>
							<button className={
								bookTypeCheckout === "audio" ? "book-overview-checkout-book-type selected" : "book-overview-checkout-book-type"
						...
						<div className="book-overview-checkout-quantity">
							...
							>
								{Array.from({ length: Math.min(10, number_of_physical_copies_available) }, (_, index) => {
									const value = index + 1;
									return (
